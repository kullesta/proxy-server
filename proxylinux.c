...
static ULONGLONG getTimeMs(void) {
    struct timespec ts;
...
static void saveReadmeFile(void) {
    struct stat st;
    ...
    if (stat("readme.txt", &st) == 0) return;
    ...
    fprintf(f, "  addblocked <host>  - add a blocked domain/IP/site\n");
    fprintf(f, "  rmblocked <host>   - remove a blocked domain/IP/site\n");
    fprintf(f, "  listblocked        - show blocked entries\n");
    fprintf(f, "  addipblack <ip>    - add a client IP to blacklist\n");
    fprintf(f, "  rmipblack <ip>     - remove a client IP from blacklist\n");
    fprintf(f, "  listipblack        - show client IP blacklist\n");
    fprintf(f, "  addipwhite <ip>    - add a client IP to whitelist\n");
    fprintf(f, "  rmipwhite <ip>     - remove a client IP from whitelist\n");
    fprintf(f, "  listipwhite        - show client IP whitelist\n");
    fprintf(f, "  enableipblack      - enable client IP blacklist\n");
    fprintf(f, "  disableipblack     - disable client IP blacklist\n");
    fprintf(f, "  enableipwhite      - enable client IP whitelist\n");
    fprintf(f, "  disableipwhite     - disable client IP whitelist\n");
    fprintf(f, "  setstats <s>       - set stats refresh interval in seconds\n");
    fprintf(f, "  reload             - reload config and blocked list from files\n");
    fprintf(f, "  resetconfig        - restore all config and blocked list to defaults\n");
    fprintf(f, "  exit               - stop the server\n\n");
...
static int addClient(SOCKET client, const struct sockaddr_in* addr) {
    ...
    for (int i = 0; i < clientCapacity; i++) {
        if (clients[i].clientSock == INVALID_SOCKET) {
            ...
            clients[i].remoteConnecting = 0;
            clients[i].initialLen = 0;
            clients[i].connectStartTick = getTimeMs();
            clients[i].lastRemoteSendTick = 0;
            clients[i].lastPingMs = 0;
            clients[i].pendingToRemoteLen = 0;
            clients[i].pendingToRemotePos = 0;
    ...
    clients[index].connectStartTick = getTimeMs();
...
static void closeClient(int index) {
    ...
    if (clients[index].remoteSock != INVALID_SOCKET) {
        close(clients[index].remoteSock);
        clients[index].remoteSock = INVALID_SOCKET;
    }
    clients[index].requestedHost[0] = '\0';
    clients[index].state = 0;
...
static int connectRemote(int index, const char* host, int port, const char* initialRequest) {
    ...
    for (struct addrinfo* p = res; p; p = p->ai_next) {
        ...
        connResult = connect(remote, p->ai_addr, p->ai_addrlen);
        if (connResult == 0) break;
        if (errno == EINPROGRESS || errno == EWOULDBLOCK) break;
        close(remote);
        remote = INVALID_SOCKET;
    }
    freeaddrinfo(res);
    ...
    if (initialRequest) {
        clients[index].lastRemoteSendTick = (DWORD)getTimeMs();
    }
...
static int completeRemoteConnect(int index) {
    ...
    if (getsockopt(c->remoteSock, SOL_SOCKET, SO_ERROR, &err, &errLen) == -1) {
        return 0;
    }
    if (err != 0) return 0;
...
int main(int argc, char* argv[]) {
    ...
    initClients();
    ULONGLONG lastStatusTick = getTimeMs();
    ULONGLONG lastIpCheckTick = lastStatusTick;
    ULONGLONG lastHourlyClearTick = lastStatusTick;
    pthread_t thread;
    int threadCreated = 0;
    ...
    while (running) {
        ...
        if (FD_ISSET(listenSock, &readSet)) {
            ...
            if (client != INVALID_SOCKET) {
                if (addClient(client, &addr) < 0) close(client);
            }
        }
        for (int i = 0; i < clientCapacity; i++) {
            if (clients[i].clientSock == INVALID_SOCKET) continue;
            ...
            if (clients[i].clientSock != INVALID_SOCKET && FD_ISSET(clients[i].clientSock, &readSet)) {
                char buffer[MAX_BUFFER];
                int received = recv(clients[i].clientSock, buffer, sizeof(buffer), 0);
                if (received == SOCKET_ERROR) {
                    if (errno != EWOULDBLOCK && errno != EAGAIN) closeClient(i);
                } else if (received == 0) {
                    closeClient(i);
                } else {
                    if (clients[i].pendingToRemoteLen > 0) {
                        if (!flushPendingRemote(i)) continue;
                    }
                    if (clients[i].state == 0) {
                        if (clients[i].initialLen + received >= MAX_BUFFER - 1) {
                            closeClient(i);
                            continue;
                        }
                        memcpy(clients[i].initialBuffer + clients[i].initialLen, buffer, received);
                        ...
                        int port = 80;
                        int isConnect = 0;
                        parseHostPort(clients[i].initialBuffer, host, sizeof(host), &port, &isConnect);
                        if (host[0] == '\0') {
                            const char* badReq = "HTTP/1.1 400 Bad Request\r\nContent-Length: 0\r\n\r\n";
                            send(clients[i].clientSock, badReq, (int)strlen(badReq), 0);
                            closeClient(i);
                            continue;
                        }
                        ...
                        const char* requestToSend = clients[i].initialBuffer;
                        if (!isConnect) {
                            if (!buildRemoteRequest(clients[i].initialBuffer, remoteRequest, sizeof(remoteRequest))) {
                                const char* badRequest = "HTTP/1.1 400 Bad Request\r\nContent-Length: 0\r\n\r\n";
                                send(clients[i].clientSock, badRequest, (int)strlen(badRequest), 0);
                                closeClient(i);
                                continue;
                            }
                            requestToSend = remoteRequest;
                        }
                        if (!connectRemote(i, host, port, isConnect ? NULL : requestToSend)) {
                            const char* badGateway = "HTTP/1.1 502 Bad Gateway\r\nContent-Length: 0\r\n\r\n";
                            send(clients[i].clientSock, badGateway, (int)strlen(badGateway), 0);
                            closeClient(i);
                            continue;
                        }
                        if (isConnect) {
                            const char* success = "HTTP/1.1 200 Connection established\r\n\r\n";
                            send(clients[i].clientSock, success, (int)strlen(success), 0);
                        }
                        clients[i].state = 1;
                        continue;
                    }
                    if (clients[i].remoteSock != INVALID_SOCKET) {
                        if (!queueToRemote(i, buffer, received)) {
                            closeClient(i);
                        } else {
                            clients[i].lastRemoteSendTick = (DWORD)getTimeMs();
                        }
                    }
                }
            }
            if (clients[i].remoteSock != INVALID_SOCKET && FD_ISSET(clients[i].remoteSock, &readSet)) {
                char buffer[MAX_BUFFER];
                int received = recv(clients[i].remoteSock, buffer, sizeof(buffer), 0);
                if (received == SOCKET_ERROR) {
                    if (errno != EWOULDBLOCK && errno != EAGAIN) closeClient(i);
                } else if (received == 0) {
                    closeClient(i);
                } else {
                    if (clients[i].pendingToClientLen > 0) {
                        if (!flushPendingClient(i)) continue;
                    }
                    if (clients[i].lastRemoteSendTick != 0) {
                        ULONGLONG now = getTimeMs();
                        clients[i].lastPingMs = (int)(now - clients[i].lastRemoteSendTick);
                        clients[i].lastRemoteSendTick = 0;
                    }
                    if (!queueToClient(i, buffer, received)) {
                        closeClient(i);
                    }
                }
            }
    ...
    for (int i = 0; i < clientCapacity; i++) {
        if (clients[i].clientSock != INVALID_SOCKET) close(clients[i].clientSock);
        if (clients[i].remoteSock != INVALID_SOCKET) close(clients[i].remoteSock);
    }
    free(clients);
    resetBlockedList();
    free(blockedEntries);
    if (listenSock != INVALID_SOCKET) close(listenSock);
    if (threadCreated) pthread_join(thread, NULL);
    pthread_mutex_destroy(&consoleLock);
    pthread_mutex_destroy(&commandLock);
